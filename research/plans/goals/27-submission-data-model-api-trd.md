...
- **Application lookup for submissions** — `appId` is required for NEW_SHORTCUT and CORRECTION; for APP_REQUEST the field stores context (the app being requested). The API must validate that the provided appId actually exists in the applications table (for NEW_SHORTCUT/CORRECTION); for APP_REQUEST, appId is nullable since the app doesn't exist yet.
