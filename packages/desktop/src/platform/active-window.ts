 ...
 * The polling service in TASK-0010 handles null results as "detection
 * unavailable" and continues operating in a degraded state.
 */
 ...
 *   3. __dirname/../native — non-webpack test runners (vitest, relative to src/)
 *
...
export function loadNativeModule(): NativeModule | null {
  ...
  if (typeof process !== 'undefined' && (process as NodeJS.Process & { resourcesPath?: string }).resourcesPath) {
    candidates.push(
      join((process as NodeJS.Process & { resourcesPath?: string }).resourcesPath!, 'native', 'kcc-native.node')
    );
  }
