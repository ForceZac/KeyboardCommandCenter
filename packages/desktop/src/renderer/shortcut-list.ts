 ...
 */
export function renderShortcutRow(shortcut: ShortcutEntry, platformSlug: string): string {
...
export function renderContextGroup(
  ...
  const rows = shortcuts.map((s) => renderShortcutRow(s, platformSlug)).join('');
  return (
 ...
 */
export function renderShortcutList(appDetail: AppDetail, platformSlug: string): string {
  const contextEntries = Object.entries(appDetail.contexts);
  if (contextEntries.length === 0) return '';

  return contextEntries
    .map(([name, shortcuts]) => renderContextGroup(name, shortcuts, platformSlug))
    .join('');
}
